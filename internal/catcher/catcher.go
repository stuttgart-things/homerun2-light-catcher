...
// reading: "$" delivers only messages pitched after the group exists. A light
// effect is a signal about now, so replaying a stream's backlog is never wanted.
...
func newRedisCatcher(
	...
	consumer, err := redisqueue.NewConsumerWithOptions(&redisqueue.ConsumerOptions{
		...
		// (e.g. a point and the match-winning point pitched milliseconds
		// apart) must reach the light in the order it happened, and a WLED
		// device can only show one effect at a time anyway.
		Concurrency: 1,
